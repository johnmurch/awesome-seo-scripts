	    ...
	    await page.setViewport({
	      ...
	      if (blockedResources.includes(req.resourceType())) {
			...
			for(var i=0;i<que.length;i++){
				...
				let headers = {}
				try {
					let response = await page.goto(que[i].url, {waitUntil: 'networkidle2', timeout: 60000});
					headers = {
							url: que[i].url,
					...
					if(chain.length>0){
							redirect=true
							chain.forEach((c) => {
								redirectChain.push({status: c._response._status, url: c.url(), headers:c._response._headers})
							})
					}
				}catch (err) {
				...
				try {
					...
					let metaDescription
					try {metaDescription = await page.evaluate(() => document.querySelector("head > meta[name='description']").content);}
			    catch (e1) {metaDescription = null;}
